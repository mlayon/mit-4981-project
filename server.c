...
void *handle_connection(void *p_client_socket)
{
    ...
    char content_len_wget[BUFSIZE];         /* storecontent length of file for wget*/
    ...
    else
    {
        ...
        int len = atoi(content_len_curl) + 1; // curl

        if (len == 1)
        { // wget
            len = atoi(content_len_wget);
        }
        ...
        char *str = buf;
        send(childfd, response, strlen(response), 0);
